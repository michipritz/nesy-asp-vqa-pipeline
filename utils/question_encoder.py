    ...
    for i, func in enumerate(program):
        ...
        if func_name in func_type["unary"]:
            if func_name == "scene":
                ...
                action_atoms.append(actions[func_name].format(T=t, T1=func["inputs"][0] + 1))
        elif func_name in func_type["binary_val"]:
            val = func["value_inputs"][0]
            action_atoms.append(actions[func_name].format(T=t, T1=func["inputs"][0] + 1, val=val))
        elif func_name in func_type["binary_in"]:
            t1 = func["inputs"][0]
            t2 = func["inputs"][1]
            if func_name in ["union", "intersect"]:
                action_atoms.append(actions[func_name].format(T=t, T1=t1 + 1, T2=t2 + 1))
            else:
                action_atoms.append(actions[func_name].format(T=t, T1=t1, T2=t2))
        else:
